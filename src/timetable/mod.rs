...
pub const PERIOD_START_TIME: [&str; 12] = [
    "08:30 AM", "09:20 AM", "10:20 AM", "11:10 AM", "12:10 PM", "01:00 PM", "02:00 PM", "02:50 PM", "03:50 PM",
    "04:40 PM", "05:40 PM", "06:30 PM",
];

pub const PERIOD_END_TIME: [&str; 12] = [
    "09:20 AM", "10:10 AM", "11:10 AM", "12:00 PM", "01:00 PM", "01:50 PM", "02:50 PM", "03:40 PM", "04:40 PM",
    "05:30 PM", "06:30 PM", "07:20 PM",
];
...
pub async fn get_std_classes() -> Result<Vec<Class>, ServerFnError> {
    ...
    let mut classes: Vec<Class> = classes_db.into_iter().filter_map(|c| c.try_into().ok()).collect();
...
pub fn TimetablePage() -> impl IntoView {
    let table_data = create_resource(|| (), |_| async move { get_std_classes().await });
    let (timetable_settings, flags) = todo!();
    ...
    view! {
        <div class="relative flex justify-between">
            ...
            <TimetableSettings>{todo!()}</TimetableSettings>
        </div>
...
fn TimetableSettings(children: Children) -> impl IntoView {
    ...
    let on_settings_unfocus = move |_| {
        if !settings_wrapper().unwrap().matches(":focus-within").unwrap() {
...
// let on_view_change = move |_| set_view(get_radio_value("view").and_then(|v| v.as_str().try_into().ok()));
// let on_time_style_change =
//     move |_| set_time_style(get_radio_value("time_style").and_then(|v| v.as_str().try_into().ok()));
